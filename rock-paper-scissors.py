...
class Main(Frame):
    ...
    def startUI(self):
        btn = Button(root, text="Камень", font=("Times New Roman", 10),
                     command=lambda x=1: self.btn_click(x))
        btn2 = Button(root, text="Ножницы", font=("Times New Roman", 10),
                      command=lambda x=2: self.btn_click(x))
        btn3 = Button(root, text="Бумага", font=("Times New Roman", 10),
    ...
    def btn_click(self, choise):
        ...
        if choise == comp_choise:
            self.drow += 1
            self.lbl.configure(text="Ничья-_-")
